...
function exist_data() {
    echo -e "*** 已安装MySQL或者Mariadb，请先卸载并删除数据目录以及配置文件！ ***\n" && exit
     read -p "请输入系统（debian=>1,ubuntu=>2,redhat7=>7,redhat8=>8版本： " mysq
     if [[ "$mysq" == 1 ]]; then
       apt install sudo -y
...
function debian_install_start() {
    ...
    sudo gpg --export --armor 467B942D3A79BD29 | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/mysql8.gpg
    sleep 10
    echo "deb https://mirrors.ustc.edu.cn/mysql-repo/apt/debian/ $(lsb_release -sc) mysql-8.0" | sudo tee /etc/apt/sources.list.d/mysql80.list
    sudo apt update
...
function ubuntu_install_start() {
    ...
    sudo gpg --export --armor 467B942D3A79BD29 | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/mysql8.gpg
    sleep 10
    echo "deb https://mirrors.ustc.edu.cn/mysql-repo/apt/ubuntu $(lsb_release -sc) mysql-8.0" | sudo tee /etc/apt/sources.list.d/mysql80.list
    sudo apt update
    sudo apt install mysql-community-server -y
    mysql -uroot -p$mysql_root_pwd -e "update mysql.user set host = '%' where user = 'root';flush privileges;"
    systemctl restart mysql
